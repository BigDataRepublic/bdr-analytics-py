...
class TestLagTransformer(unittest.TestCase):
    ...
    def test_lagtransformer_on_numpy(self):
        ...
        lagged = PdLagTransformer(1).fit_transform(orig_data)
        np.testing.assert_array_equal(lagged.columns, ["0_lag1", "1_lag1", "2_lag1"])
        np.testing.assert_array_equal(lagged.iloc[1, :], orig_data[0, :])
        np.testing.assert_array_equal(lagged.iloc[0, :], np.repeat(np.nan, 3))

    def test_windowtransformer(self):
        orig_data = pd.DataFrame(data=np.arange(14, -1, -1).reshape(5, 3), columns=["col1", "col2", "col3"])
        result = PdWindowTransformer(lambda window: window.max(), window = 2).fit_transform(orig_data)
        np.testing.assert_array_equal(result.columns, ["col1_window2", "col2_window2", "col3_window2"])
        np.testing.assert_array_equal(result.iloc[0, :], np.repeat(np.nan, 3))
        np.testing.assert_array_equal(result.iloc[1, :], orig_data.iloc[0, :]) # orig data is [ [14, 13, 12], [11, 10, 9],.., thus rolling max at row 1 should be values of row 0

    def test_windowtransformer_on_numpy(self):
        orig_data = np.arange(14, -1, -1).reshape(5, 3)
        result = PdWindowTransformer(lambda window: window.max(), window = 2).fit_transform(orig_data)
        np.testing.assert_array_equal(result.columns, ["0_window2", "1_window2", "2_window2"])
        np.testing.assert_array_equal(result.iloc[0, :], np.repeat(np.nan, 3))
        np.testing.assert_array_equal(result.iloc[1, :], orig_data[0, :]) # orig data is [ [14, 13, 12], [11, 10, 9],.., thus rolling max at row 1 should be values of row 0
